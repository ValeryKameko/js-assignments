...
function getRegexForGuid() {
    var hexadecimalRegExp = "[\\dA-Fa-f]";
    var guidRegExp = new RegExp('^\{' + [8, 4, 4, 4, 12].map(cnt => `${hexadecimalRegExp}\{${cnt}\}`).join('-') + '\}$');
    return guidRegExp;
}
...
function getRegexForIPv4() {
    var less256RegExp = '(\\d|[0-1]?\\d\\d|2[0-4]\\d|25[0-5])';
    return new RegExp(`^(${less256RegExp}\\.)\{3\}${less256RegExp}$`);
}
